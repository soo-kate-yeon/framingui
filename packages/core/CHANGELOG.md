...
- Added bundled `podcast-ambient` theme metadata and token payload so `loadTheme()` and `listThemes()` expose the new podcast-focused visual profile to downstream apps.
