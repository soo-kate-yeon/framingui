...
- **tekton-ui** provides an MCP server that exposes your design system to Claude Code, Cursor, and other MCP-compatible tools
...
## Setting Up tekton-ui with MCP

### Prerequisites

- Node.js 20+ installed
- Claude Code, Cursor, or any MCP-compatible AI tool
- A project with design tokens (or generate them with tekton-ui)

### Step 1: Install tekton-ui

```bash
# Install tekton-ui core packages
npm install @tekton/core @tekton/mcp-server

# Or with yarn
yarn add @tekton/core @tekton/mcp-server

# Or with pnpm
pnpm add @tekton/core @tekton/mcp-server
```

### Step 2: Generate Design Tokens (Optional)

If you don't have design tokens yet, tekton-ui can generate them:

```bash
# Initialize tekton-ui with interactive prompts
npx @tekton/cli init
...
Type `@tekton` in chat to see available MCP commands:
- `@tekton/tokens` - List all tokens
- `@tekton/components` - List component knowledge
- `@tekton/validate` - Validate code against design system
...
tekton-ui's MCP server exposes not just tokens but **component knowledge**—semantic metadata about when and how to use components.
...
You don't need to use tekton-ui's token generator—MCP works with any design token format.

### From Figma Tokens

If you use the Figma Tokens plugin:

```bash
# Export tokens from Figma as JSON
# Place in project root as design-tokens.json

# Configure MCP to read Figma format
```
...
// .claude/mcp.json
{
  "servers": {
    "tekton-design-tokens": {
      "command": "npx",
      "args": ["@tekton/mcp-server", "--format", "figma"],
      "env": {
...
```json
{
  "servers": {
    "tekton-design-tokens": {
      "command": "npx",
      "args": ["@tekton/mcp-server", "--format", "style-dictionary"],
      "env": {
...
npx @tekton/cli extract-tailwind
...
Yes! MCP is an open standard. Any tool that supports Model Context Protocol can connect to tekton-ui's MCP server. Currently supported:
...
Not yet. MCP currently provides read-only access to design tokens. Token generation still requires using tekton-ui CLI or your existing design system workflow. Bi-directional MCP (read + write) is planned for a future release.

### Does this work with component libraries like shadcn/ui?

Yes! tekton-ui's MCP server includes component knowledge for shadcn/ui components. When you ask AI to generate a component, it automatically:
...
## Get Started with tekton-ui MCP

Ready to connect your design system to AI?

### Option 1: Try the Playground

Visit [tekton-ui.com/studio](https://tekton-ui.com/studio) to:
...
- [MCP Integration Guide](https://tekton-ui.com/docs/mcp) — Complete setup documentation
- [API Reference](https://tekton-ui.com/docs/api) — MCP server API specification
- [Component Knowledge](https://tekton-ui.com/docs/components) — Available components and metadata

**Community:**

- [GitHub Discussions](https://github.com/tekton-ui/discussions) — Ask questions
- [Discord](https://tekton-ui.com/discord) — Real-time help from the community

---

*Built with [tekton-ui](https://tekton-ui.com) — AI-powered design systems with MCP integration.*
