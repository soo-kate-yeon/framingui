        ...
        <div className="min-h-screen bg-[#F0F0F0] font-sans selection:bg-neutral-900 selection:text-white flex">
            ...
            <aside className="w-[280px] bg-white border-r border-neutral-200 flex flex-col fixed h-full z-20">
                <div className="p-8 border-b border-neutral-100">
                    ...
                    <span className="text-[10px] uppercase tracking-widest text-neutral-400 mt-1 block">Workspace</span>
                </div>
                ...
                <div className="p-8 border-t border-neutral-100">
                    <div className="bg-neutral-50 p-4 border border-neutral-100">
                        <div className="text-xs font-bold uppercase tracking-wide text-neutral-500 mb-2">My Plan</div>
...
function NavItem({ href, icon, label, active = false }: { href: string, icon: React.ReactNode, label: string, active?: boolean }) {
    return (
        <Link href={href} className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-medium transition-colors ${active ? 'bg-neutral-900 text-white' : 'text-neutral-500 hover:text-neutral-900 hover:bg-neutral-50'}`}>
            {icon}
            {label}
        </Link>
    )
