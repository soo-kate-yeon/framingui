...
const CLASSIC_MAGAZINE_FALLBACK: Record<string, string> = {
  ...
  '--bg-card': '#FAFAFA',
  '--bg-card-foreground': '#1A1A1A',
  '--bg-popover': '#FAFAFA',
  '--bg-popover-foreground': '#1A1A1A',
  '--bg-primary': '#1A1A1A',
  '--bg-primary-foreground': '#FFFFFF',
  '--bg-secondary': '#F5F5F5',
  '--bg-secondary-foreground': '#1A1A1A',
  '--bg-muted': '#F5F5F5',
  '--bg-muted-foreground': '#666666',
  '--bg-accent': '#F5F5F5',
  '--bg-accent-foreground': '#1A1A1A',
...
function ClassicMagazineDemoInner() {
  ...
  return (
    ...
    >
      {!isEmbed && <PreviewBanner templateId="classic-magazine" templateName="Classic Magazine" />}
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute inset-x-0 top-[104px] bottom-0 bg-[var(--bg-surface)] z-50 overflow-y-auto shadow-xl">
          <nav className="flex flex-col gap-2 p-4">
            ...
            <button
              onClick={() => router.push('/explore/classic-magazine/article')}
              className="text-lg font-medium px-4 py-4 rounded-[var(--radius-lg)] transition-colors text-left text-[var(--text-secondary)] flex items-center justify-between"
            >
              <span>{locale === 'ko' ? '에디토리얼 기사' : 'Editorial Article'}</span>
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button className="flex-1 sm:flex-none h-9 px-4 py-2 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-[var(--radius-md)] text-xs font-bold uppercase tracking-wider hover:bg-[var(--bg-canvas)] transition-colors">
            Download Export
          </button>
          <button className="flex-1 sm:flex-none h-9 px-4 py-2 bg-[var(--action-primary)] text-[var(--action-primary-text)] rounded-[var(--radius-md)] text-xs font-bold uppercase tracking-wider hover:opacity-90 transition-opacity">
            New Project
          </button>
        </div>
      </div>
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
        {[
          ...
          <div
            key={stat.title}
            className="rounded-[var(--radius-lg)] border border-[var(--border-default)] border-t-2 border-t-[var(--border-emphasis)] bg-[var(--bg-surface)] text-[var(--text-primary)] shadow-sm min-w-0"
          >
            <div className="p-4 md:p-6 flex flex-row items-center justify-between space-y-0 pb-2">
              <h3 className="tracking-widest uppercase text-[10px] md:text-xs font-bold text-[var(--text-secondary)] truncate pr-2">
                {stat.title}
              </h3>
      ...
      <div className="grid gap-4 grid-cols-1 lg:grid-cols-7">
        <div className="lg:col-span-4 rounded-[var(--radius-lg)] border border-[var(--border-default)] border-t-2 border-t-[var(--border-emphasis)] bg-[var(--bg-surface)] text-[var(--text-primary)] shadow-sm overflow-hidden">
        ...
        <div className="lg:col-span-3 rounded-[var(--radius-lg)] border border-[var(--border-default)] border-t-2 border-t-[var(--border-emphasis)] bg-[var(--bg-surface)] text-[var(--text-primary)] shadow-sm">
      ...
      <div className="rounded-[var(--radius-lg)] border border-[var(--border-default)] border-t-2 border-t-[var(--border-emphasis)] bg-[var(--bg-surface)] shadow-sm">
        <div className="flex flex-col space-y-1.5 p-6">
          <h3 className="font-serif text-lg font-semibold leading-none tracking-tight">
            Recent Transactions
          </h3>
        ...
        <div className="px-6 pb-6">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b-2 border-[var(--border-emphasis)]">
                  <th className="text-left text-xs font-bold uppercase tracking-wider text-[var(--text-secondary)] pb-3">
                    Invoice
                  </th>
                  <th className="text-left text-xs font-bold uppercase tracking-wider text-[var(--text-secondary)] pb-3">
                    Status
                  ...
                  <th className="text-right text-xs font-bold uppercase tracking-wider text-[var(--text-secondary)] pb-3">
                    Amount
                  </th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  >
                    ...
                    <td className="py-3">
                      <span
                        className={`inline-flex items-center rounded-[var(--radius-full)] px-2.5 py-0.5 text-xs font-medium ${tx.status === 'Paid' ? 'bg-green-100 text-green-800' : tx.status === 'Pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}
                      >
                        {tx.status}
                      </span>
                    </td>
