...
function MCPAuthContent() {
  ...
  // 유효성 검사
  const isValidPort = callbackPort && /^\d+$/.test(callbackPort);
  const isValidState = state && state.length >= 32;

  if (!isValidPort || !isValidState) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50">
        <div className="w-full max-w-md rounded-lg bg-white p-8 shadow-md">
          <h1 className="mb-4 text-2xl font-bold text-red-600">Invalid Request</h1>
          <p className="text-gray-600">
            Missing or invalid parameters. Please run <code className="rounded bg-gray-100 px-2 py-1">tekton-mcp login</code> from your terminal.
          </p>
        </div>
      </div>
    );
  ...
  // OAuth 버튼 핸들러 - /api/mcp/cli-callback 경유
  const handleOAuth = (provider: 'google' | 'github') => {
    ...
    window.location.href = `/api/mcp/cli-callback?${params.toString()}`;
  };
      ...
      <div className="w-full max-w-md rounded-lg bg-white p-8 shadow-md">
        <div className="mb-6 text-center">
          ...
          <p className="text-sm text-gray-500">
            Sign in to connect your development tools
          </p>
        </div>
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-3 rounded-lg border border-gray-300 bg-gray-900 px-4 py-3 text-sm font-medium text-white transition-colors hover:bg-gray-800"
          >
