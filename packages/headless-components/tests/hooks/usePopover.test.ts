...
describe("usePopover", () => {
  ...
  describe("Initialization", () => {
    ...
    it("should generate unique IDs", () => {
      ...
      expect(result1.current.popoverProps.id).not.toBe(
        result2.current.popoverProps.id,
      );
    });
  ...
  describe("Click Trigger", () => {
    ...
    it("should call onOpenChange callback", () => {
      ...
      const { result } = renderHook(() =>
        usePopover({ trigger: "click", onOpenChange }),
      );
  ...
  describe("Escape Key", () => {
    it("should close on Escape key when enabled", () => {
      const { result } = renderHook(() =>
        usePopover({ closeOnEscape: true, defaultOpen: true }),
      );
