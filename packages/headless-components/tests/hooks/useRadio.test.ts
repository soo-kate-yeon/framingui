...
describe("useRadioGroup", () => {
  ...
  describe("Initialization", () => {
    ...
    it("should initialize with defaultValue", () => {
      const { result } = renderHook(() =>
        useRadioGroup({ defaultValue: "option1" }),
      );
  ...
  describe("Controlled Mode", () => {
    ...
    it("should call onChange when value changes", () => {
      ...
      const { result: groupResult } = renderHook(() =>
        useRadioGroup({ value: "option1", onChange }),
      );

      const radioInputProps = groupResult.current.getRadioProps({
        value: "option2",
      });
      const { result: radioResult } = renderHook(() =>
        useRadio(radioInputProps),
      );
  ...
  describe("ARIA Attributes", () => {
    ...
    it("should include aria-label on group when provided", () => {
      const { result } = renderHook(() =>
        useRadioGroup({ "aria-label": "Choose option" }),
      );
