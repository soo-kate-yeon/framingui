...
describe('useProgress', () => {
  ...
  describe('Determinate Mode', () => {
    it('should calculate percentage correctly', () => {
      const { result } = renderHook(() => useProgress({ value: 45, min: 0, max: 100 }));
    ...
    it('should handle custom min and max', () => {
      const { result } = renderHook(() => useProgress({ value: 50, min: 0, max: 200 }));
      expect(result.current.percentage).toBe(25);
    });

    it('should clamp value within min/max', () => {
      const { result: below } = renderHook(() => useProgress({ value: -10, min: 0, max: 100 }));
      expect(below.current.value).toBe(0);

      const { result: above } = renderHook(() => useProgress({ value: 150, min: 0, max: 100 }));
  ...
  describe('Completion', () => {
    it('should set isComplete=true when value >= max', () => {
      const { result } = renderHook(() => useProgress({ value: 100, max: 100 }));
  ...
  describe('ARIA Attributes', () => {
    ...
    it('should include aria-label when provided', () => {
      const { result } = renderHook(() => useProgress({ value: 50, ariaLabel: 'Loading progress' }));
      expect(result.current.progressProps['aria-label']).toBe('Loading progress');
    });

    it('should include aria-labelledby when provided', () => {
      const { result } = renderHook(() => useProgress({ value: 50, ariaLabelledBy: 'progress-label' }));
      expect(result.current.progressProps['aria-labelledby']).toBe('progress-label');
    });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle negative values', () => {
      const { result } = renderHook(() => useProgress({ value: -10, min: 0, max: 100 }));
      expect(result.current.value).toBe(0);
      expect(result.current.percentage).toBe(0);
    });

    it('should handle values exceeding max', () => {
      const { result } = renderHook(() => useProgress({ value: 150, min: 0, max: 100 }));
