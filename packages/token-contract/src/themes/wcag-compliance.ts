 ...
 */
function calculateContrastRatio(color1: ColorToken, color2: ColorToken): number {
  const lum1 = calculateRelativeLuminance(oklchToRGB(color1));
  const lum2 = calculateRelativeLuminance(oklchToRGB(color2));
...
export function validateWCAGCompliance(
  ...
  const lightBg = theme.tokens.neutral['50'];
  const darkBg = theme.tokens.neutral['900'];

  if (!lightBg || !darkBg) {
    throw new Error('Theme missing required neutral background colors');
  }
  ...
  semanticColors.forEach((semantic) => {
    const scale = theme.tokens[semantic];
    if (!scale) return;
    ...
    testSteps.forEach((step) => {
      ...
      if (!passedLight) violations.push(checkLight);
      ...
      if (!passedDark) violations.push(checkDark);
    });
  });
