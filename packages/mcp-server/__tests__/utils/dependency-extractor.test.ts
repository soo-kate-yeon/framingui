...
describe('extractDependencies', () => {
  ...
  describe('내부 패키지 추출', () => {
    it('@tekton 패키지를 internal로 추출해야 함', () => {
      const code = `
        ...
        import { resolveScreen } from '@tekton/core';
      `;
      const result = extractDependencies(code);

      expect(result.internal).toContain('@tekton/ui');
      expect(result.internal).toContain('@tekton/core');
      expect(result.internal).toHaveLength(2);
      expect(result.external).not.toContain('@tekton/ui');
      expect(result.external).not.toContain('@tekton/core');
    });
  });
  ...
  describe('서브패스에서 패키지 이름 추출', () => {
    it('서브패스 import에서 패키지 이름을 추출해야 함', () => {
      const code = `
        import { Button } from '@tekton/ui/components/Button';
        import { motion } from 'framer-motion/dist/framer-motion';
      `;
      const result = extractDependencies(code);

      expect(result.internal).toContain('@tekton/ui');
      expect(result.internal).not.toContain('@tekton/ui/components/Button');
      expect(result.external).toContain('framer-motion');
  ...
  describe('다중 패키지 및 중복 제거', () => {
    it('여러 패키지를 처리하고 중복을 제거해야 함', () => {
      const code = `
        ...
        import { Button } from '@tekton/ui';
        import { Card } from '@tekton/ui/components/Card';
      `;
      const result = extractDependencies(code);

      expect(result.external).toEqual(['framer-motion']);
      expect(result.internal).toEqual(['@tekton/ui']);
    });
  ...
  describe('복잡한 시나리오', () => {
    ...
    it('TypeScript 코드를 올바르게 파싱해야 함', () => {
      const code = `
        ...
        import { useState } from 'react';
        import type { User } from '@tekton/core/types';
        import { motion } from 'framer-motion';
      ...
      expect(result.internal).toEqual(['@tekton/core']);
    });
